...
export default function StatisticsChart({
  loading,
  series,
  subtitle = "Your portfolio activity by month (current year)",
}: Props) {
  ...
  const chartSeries = useMemo(() => {
    if (series?.length && series[0]?.data?.length === 12) {
      return series;
    }
    return [
      { name: "New listings", data: Array(12).fill(0) },
      { name: "Transaction volume (k)", data: Array(12).fill(0) },
    ];
  }, [series]);
  ...
  if (loading) {
    return (
      <div className="rounded-2xl border border-gray-200 bg-white px-5 pb-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6 animate-pulse">
        <div className="h-6 w-32 rounded bg-gray-200 dark:bg-gray-700 mb-2" />
        <div className="h-4 w-64 rounded bg-gray-100 dark:bg-gray-800 mb-6" />
        <div className="h-[310px] rounded-lg bg-gray-100 dark:bg-gray-800" />
      </div>
    );
  }
